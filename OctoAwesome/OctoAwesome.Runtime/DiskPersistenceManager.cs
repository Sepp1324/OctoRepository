...
namespace OctoAwesome.Runtime
{
    ...
    public class DiskPersistenceManager : IPersistenceManager, IDisposable, INotificationObserver
    {
        ...
        {
            return new ComponentContainerDbContext<IEntityComponent>(databaseProvider, universeGuid)
                .GetComponentContainerWithComponent<T>().OfType<Entity>();
        }

        public IEnumerable<Guid> GetEntityIdsFromComponent<T>(Guid universeGuid) where T : IEntityComponent
        {
            return new ComponentContainerDbContext<IEntityComponent>(databaseProvider, universeGuid)
                .GetComponentContainerIdsFromComponent<T>().Select(i => i.Tag);
        }

        public IEnumerable<Guid> GetEntityIds(Guid universeGuid)
        {
            return new ComponentContainerDbContext<IEntityComponent>(databaseProvider, universeGuid).GetAllKeys()
                .Select(i => i.Tag);
        }
            ...
            where T : IEntityComponent, new()
        {
            foreach (var entityId in entityIds)
                yield return (entityId,
                    new ComponentContainerComponentDbContext<T>(databaseProvider, universeGuid).Get<T>(entityId));
        }
