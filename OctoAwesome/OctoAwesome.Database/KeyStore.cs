...
namespace OctoAwesome.Database
{
    internal class KeyStore<TTag> : IDisposable where TTag : ITag, new()
    {
        ...
        public void Open()
        {
            ...
            for (var i = 0; i < buffer.Length; i += Key<TTag>.KeySize)
            {
        ...
        internal void Update(Key<TTag> key)
        {
            ...
            _writer.WriteAndFlush(key.GetBytes(), 0, Key<TTag>.KeySize, oldKey.Position);
        }
        ...
        internal void Add(Key<TTag> key)
        {
            ...
            _keys.Add(key.Tag, key);
            _writer.WriteAndFlush(key.GetBytes(), 0, Key<TTag>.KeySize);
        }
        ...
        internal void Remove(TTag tag, out Key<TTag> key)
        {
            key = _keys[tag];
            _keys.Remove(tag);
            _writer.WriteAndFlush(Key<TTag>.Empty.GetBytes(), 0, Key<TTag>.KeySize, key.Position);
        }
    }
}