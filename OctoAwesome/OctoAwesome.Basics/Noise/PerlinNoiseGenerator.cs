...
namespace OctoAwesome.Noise
{
    public class PerlinNoiseGenerator : INoise
    {
        ...
        private float LinearInterpolation3(float a, float b, float c, float d, float e, float f, float g, float h,
        ...
        public float SmoothedNoise2(int x, int y)
        {
            ...
            var sides = (Noise2(x + 1, y) + Noise2(x - 1, y) + Noise2(x, y + 1) + Noise2(x, y - 1)) /
                        (4 * Smoothfactor);
            var corners = (Noise2(x + 1, y + 1) + Noise2(x - 1, y - 1) + Noise2(x - 1, y + 1) + Noise2(x + 1, y - 1)) /
                          (4 * (float)Math.Sqrt(2) * Smoothfactor);
        ...
        public float SmoothedNoise3(int x, int y, int z)
        {
            ...
            var directSides =
                (Noise3(x + 1, y, z) + Noise3(x - 1, y, z) + Noise3(x, y + 1, z) + Noise3(x, y - 1, z) +
                 Noise3(x, y, z - 1) + Noise3(x, y, z + 1)) / (6 * Smoothfactor);

            var indirectSides = (Noise3(x + 1, y + 1, z) + Noise3(x - 1, y - 1, z) + Noise3(x - 1, y + 1, z) +
                                 Noise3(x + 1, y - 1, z) +
                                 Noise3(x + 1, y, z - 1) + Noise3(x - 1, y, z - 1) + Noise3(x, y + 1, z - 1) +
                                 Noise3(x, y - 1, z - 1) +
                                 Noise3(x + 1, y, z + 1) + Noise3(x - 1, y, z + 1) + Noise3(x, y + 1, z + 1) +
                                 Noise3(x, y - 1, z + 1)) /
                                (12 * (float)Math.Sqrt(2) * Smoothfactor);

            var corners = (Noise3(x + 1, y + 1, z - 1) + Noise3(x - 1, y - 1, z - 1) + Noise3(x - 1, y + 1, z - 1) +
                           Noise3(x + 1, y - 1, z - 1) +
        ...
        public float[,] PerlinNoise2(int startX, int startY, int width, int heigth)
        {
            ...
            for (var i = 0; i < Octaves; i++)
            {
                ...
                for (var y = 0; y < heigth; y++)
                    noiseLayers[i, x, y] = InterpolatedSNoise2((float)(x + startX) / Sizefactor * frequency,
                        (float)(y + startY) / Sizefactor * frequency) * amplitude;
            }
        ...
        public float[,,] PerlinNoise3(int startX, int startY, int startZ, int width, int heigth, int depth)
        {
            ...
            for (var i = 0; i < Octaves; i++)
            {
                ...
                for (var z = 0; z < depth; z++)
                    noiseLayers[i, x, y, z] = InterpolatedSNoise3((float)(x + startX) / Sizefactor * frequency,
                                                  (float)(y + startY) / Sizefactor * frequency,
                                                  (float)(z + startZ) / Sizefactor * frequency) *
                                              amplitude;
            }
