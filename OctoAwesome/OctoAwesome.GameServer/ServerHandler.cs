...
namespace OctoAwesome.GameServer
{
    public class ServerHandler : IAsyncObserver<Package>
    {
        ...
        public ServerHandler()
        {
            ...
            _defaultManager = new DefaultCommandManager<ushort, CommandParameter, byte[]>(typeof(ServerHandler).Namespace + ".Commands");
        }

        private SimulationManager SimulationManager { get; }
        ...
        public async Task OnNext(Package value)
        {
            ...
            try
            {
                value.Payload = _defaultManager.Dispatch(value.Command, new CommandParameter(value.BaseClient.Id, value.Payload));
            }
            catch (Exception ex)
            {
