...
namespace OctoAwesome.Network
{
    public class NetworkPersistenceManager : IPersistenceManager, IAsyncObserver<Package>
    {
        ...
        public IEnumerable<(Guid Id, T Component)> GetEntityComponents<T>(Guid universeGuid, Guid[] entityIds) where T : EntityComponent, new() => Array.Empty<(Guid, T)>();
        ...
        private Awaiter GetAwaiter(ISerializable serializable, uint packageUId)
        {
            ...
            if (!_packages.TryAdd(packageUId, awaiter)) _logger.Error($"Awaiter for package {packageUId} could not be added");
