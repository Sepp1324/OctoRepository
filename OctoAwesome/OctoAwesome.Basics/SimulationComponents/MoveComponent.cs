...
namespace OctoAwesome.Basics.SimulationComponents
{
        ...
        PositionComponent>
    {
        protected override SimulationComponentRecord<Entity, MoveableComponent, PositionComponent> OnAdd(Entity entity)
        {
            ...
            return new SimulationComponentRecord<Entity, MoveableComponent, PositionComponent>(entity, movecomp,
                poscomp);
        }

        protected override void UpdateValue(GameTime gameTime,
            SimulationComponentRecord<Entity, MoveableComponent, PositionComponent> value)
        {
            ...
            if (Math.Abs(tmp.X) < 0.01) tmp.X = 0;
            if (Math.Abs(tmp.Y) < 0.01) tmp.Y = 0;
        ...
        private void CheckBoxCollision(GameTime gameTime, Entity entity, MoveableComponent movecomp,
            PositionComponent poscomp)
        {
            ...
            var playerplanes =
                CollisionPlane.GetEntityCollisionPlanes(bc.Radius, bc.Height, movecomp.Velocity, poscomp.Position);
            ...
            for (var x = minx; x <= maxx && !abort; x++)
            {
                ...
                var poolingList = new PoolingList<CollisionPlane>();
                ...
                foreach (var blockPlane in poolingList)
                {
                    ...
                    if (blockPlane.normal.X != 0 && (movecomp.Velocity.X > 0 && diff.X >= 0 && subvelocity.X >= 0 ||
                                                     movecomp.Velocity.X < 0 && diff.X <= 0 && subvelocity.X <= 0))
                        vx = subvelocity.X;
                    else
                        vx = movecomp.Velocity.X;

                    if (blockPlane.normal.Y != 0 && (movecomp.Velocity.Y > 0 && diff.Y >= 0 && subvelocity.Y >= 0 ||
                                                     movecomp.Velocity.Y < 0 && diff.Y <= 0 && subvelocity.Y <= 0))
                        vy = subvelocity.Y;
                    else
                        vy = movecomp.Velocity.Y;

                    if (blockPlane.normal.Z != 0 && (movecomp.Velocity.Z > 0 && diff.Z >= 0 && subvelocity.Z >= 0 ||
                                                     movecomp.Velocity.Z < 0 && diff.Z <= 0 && subvelocity.Z <= 0))
                        vz = subvelocity.Z;
                    else
