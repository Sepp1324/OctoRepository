...
namespace OctoAwesome.Client.Controls
{
    internal sealed class SceneControl : Control, IDisposable
    {
        ...
        private Index2 currentChunk = new(-1, -1);
            ...
            base(manager, style)
        {
            ...
            var tmpSphereRadius = (float)(Math.Sqrt(Span * Chunk.CHUNKSIZE_X * Span * Chunk.CHUNKSIZE_X * 3) / 3 +
                                          camera.NearPlaneDistance + chunkDiag / 2);
            ...
            var textureCount = 0;
            foreach (var definition in definitions) textureCount += definition.Textures.Length;
            var bitmapSize = 128;
            ...
            foreach (var bitmap in definition.Textures)
            {
                ...
                var bitmapData = scaled.LockBits(new System.Drawing.Rectangle(0, 0, scaled.Width, scaled.Height),
            ...
            for (var i = 0; i < chunkRenderer.GetLength(0); i++)
            for (var j = 0; j < chunkRenderer.GetLength(1); j++)
            {
                var renderer = new ChunkRenderer(this, Manager.Game.DefinitionManager, simpleShader,
                    manager.GraphicsDevice, camera.Projection, blockTextures);
            ...
            for (var i = 0; i < additional; i++)
            {
                ...
                var t = new Task(AdditionalFillerBackgroundLoop, (are, i, token), token,
            ...
            {
                new VertexPositionTexture(new Vector3(-0.5f, 0.5f), new Vector2(0, 0)),
                new VertexPositionTexture(new Vector3(0.5f, 0.5f), new Vector2(1, 0)),
                new VertexPositionTexture(new Vector3(-0.5f, -0.5f), new Vector2(0, 1)),
                new VertexPositionTexture(new Vector3(0.5f, 0.5f), new Vector2(1, 0)),
                new VertexPositionTexture(new Vector3(0.5f, -0.5f), new Vector2(1, 1)),
                new VertexPositionTexture(new Vector3(-0.5f, -0.5f), new Vector2(0, 1))
            };
            ...
            selectionLines = new VertexBuffer(manager.GraphicsDevice, VertexPositionColor.VertexDeclaration,
                selectionVertices.Length);
            selectionLines.SetData(selectionVertices);

            selectionIndexBuffer = new IndexBuffer(manager.GraphicsDevice, DrawElementsType.UnsignedShort,
                selectionIndices.Length);
            selectionIndexBuffer.SetData(selectionIndices);

            billboardVertexbuffer = new VertexBuffer(manager.GraphicsDevice, VertexPositionTexture.VertexDeclaration,
            ...
            MiniMapTexture =
                new RenderTarget2D(manager.GraphicsDevice, 128, 128,
                    PixelInternalFormat
                        .Rgb8); // , false, SurfaceFormat.Color, DepthFormat.Depth24Stencil8, 0, RenderTargetUsage.PreserveContents);
        ...
        protected override void OnUpdate(GameTime gameTime)
        {
            ...
            var centerblock = player.Position.Position.GlobalBlockIndex;
            var renderOffset = player.Position.Position.ChunkIndex * Chunk.CHUNKSIZE;

            Index3? selected;
            Axis? selectedAxis;
            Vector3? selectionPoint;

            var selBlock = GetSelectedBlock(centerblock, renderOffset, out selected, out selectedAxis,
                out selectionPoint);
            var funcBlock = GetSelectedFunctionalBlock(centerblock, renderOffset, out var selectedFunc,
                out var selectedFuncAxis, out var selectionFuncPoint);
            ...
            if (selected.HasValue && selectionPoint.HasValue)
            {
                ...
                {
                    case Axis.X:
                        player.SelectedSide = camera.PickRay.Direction.X > 0
                            ? OrientationFlags.SideWest
                            : OrientationFlags.SideEast;
                        break;
                    case Axis.Y:
                        player.SelectedSide = camera.PickRay.Direction.Y > 0
                            ? OrientationFlags.SideSouth
                            : OrientationFlags.SideNorth;
                        break;
                    case Axis.Z:
                        player.SelectedSide = camera.PickRay.Direction.Z > 0
                ...
                switch (player.SelectedSide)
                {
                    case OrientationFlags.SideWest:
                        ...
                        player.SelectedCorner = FindCorner(player.SelectedPoint.Value, OrientationFlags.Corner011,
                            OrientationFlags.Corner001, OrientationFlags.Corner010, OrientationFlags.Corner000);
                        player.SelectedEdge = FindEdge(player.SelectedPoint.Value, OrientationFlags.EdgeWestTop,
                            OrientationFlags.EdgeWestBottom, OrientationFlags.EdgeNorthWest,
                            OrientationFlags.EdgeSouthWest);
                        break;
                    case OrientationFlags.SideEast:
                        player.SelectedPoint = new Vector2(selectionPoint.Value.Y, 1f - selectionPoint.Value.Z);
                        player.SelectedCorner = FindCorner(player.SelectedPoint.Value, OrientationFlags.Corner101,
                            OrientationFlags.Corner111, OrientationFlags.Corner100, OrientationFlags.Corner110);
                        player.SelectedEdge = FindEdge(player.SelectedPoint.Value, OrientationFlags.EdgeEastTop,
                            OrientationFlags.EdgeEastBottom, OrientationFlags.EdgeSouthEast,
                            OrientationFlags.EdgeNorthEast);
                        break;
                    case OrientationFlags.SideTop:
                        player.SelectedPoint = new Vector2(selectionPoint.Value.X, 1f - selectionPoint.Value.Y);
                        player.SelectedCorner = FindCorner(player.SelectedPoint.Value, OrientationFlags.Corner011,
                            OrientationFlags.Corner111, OrientationFlags.Corner001, OrientationFlags.Corner101);
                        player.SelectedEdge = FindEdge(player.SelectedPoint.Value, OrientationFlags.EdgeNorthTop,
                            OrientationFlags.EdgeSouthTop, OrientationFlags.EdgeWestTop, OrientationFlags.EdgeEastTop);
                        break;
                    case OrientationFlags.SideBottom:
                        player.SelectedPoint = new Vector2(selectionPoint.Value.X, selectionPoint.Value.Y);
                        player.SelectedCorner = FindCorner(player.SelectedPoint.Value, OrientationFlags.Corner000,
                            OrientationFlags.Corner100, OrientationFlags.Corner010, OrientationFlags.Corner110);
                        player.SelectedEdge = FindEdge(player.SelectedPoint.Value, OrientationFlags.EdgeSouthBottom,
                            OrientationFlags.EdgeNorthBottom, OrientationFlags.EdgeWestBottom,
                            OrientationFlags.EdgeEastBottom);
                        break;
                    case OrientationFlags.SideNorth:
                        player.SelectedPoint = new Vector2(1f - selectionPoint.Value.X, 1f - selectionPoint.Value.Z);
                        player.SelectedCorner = FindCorner(player.SelectedPoint.Value, OrientationFlags.Corner111,
                            OrientationFlags.Corner011, OrientationFlags.Corner110, OrientationFlags.Corner010);
                        player.SelectedEdge = FindEdge(player.SelectedPoint.Value, OrientationFlags.EdgeNorthTop,
                            OrientationFlags.EdgeNorthBottom, OrientationFlags.EdgeNorthEast,
                            OrientationFlags.EdgeNorthWest);
                        break;
                    case OrientationFlags.SideSouth:
                        player.SelectedPoint = new Vector2(selectionPoint.Value.X, 1f - selectionPoint.Value.Z);
                        player.SelectedCorner = FindCorner(player.SelectedPoint.Value, OrientationFlags.Corner001,
                            OrientationFlags.Corner101, OrientationFlags.Corner000, OrientationFlags.Corner100);
                        player.SelectedEdge = FindEdge(player.SelectedPoint.Value, OrientationFlags.EdgeSouthTop,
                            OrientationFlags.EdgeSouthBottom, OrientationFlags.EdgeSouthWest,
                            OrientationFlags.EdgeSouthEast);
                        break;
                }
            ...
            var destinationChunk = new Index2(player.Position.Position.ChunkIndex);

            // Nur ausführen wenn der Spieler den Chunk gewechselt hat
            if (destinationChunk != currentChunk) fillResetEvent.Set();

            base.OnUpdate(gameTime);
        }

        private BlockInfo GetSelectedBlock(Index3 centerblock, Index3 renderOffset, out Index3? selected,
            out Axis? selectedAxis, out Vector3? selectionPoint)
        {
            selected = null;
            selectedAxis = null;
            selectionPoint = null;
            ...
            for (var x = -Player.SELECTIONRANGE; x < Player.SELECTIONRANGE; x++)
            {
                ...
                var distance = Block.Intersect(blockDefinition.GetCollisionBoxes(localChunkCache, pos.X, pos.Y, pos.Z),
                    pos - renderOffset, camera.PickRay, out var collisionAxis);
                ...
                if (distance.HasValue && distance.Value < bestDistance)
                {
                    ...
                    selectionPoint = camera.PickRay.Position + camera.PickRay.Direction * distance -
        ...
        private FunctionalBlock GetSelectedFunctionalBlock(Index3 centerblock, Index3 renderOffset,
            out Index3? selected, out Axis? selectedAxis, out Vector3? selectionPoint)
        {
            selected = null;
            selectedAxis = null;
            selectionPoint = null;
        ...
        protected override void OnPreDraw(GameTime gameTime)
        {
            ...
            var playerPosX = player.Position.Position.GlobalPosition.X / (planet.Size.X * Chunk.CHUNKSIZE_X) *
                             MathHelper.TwoPi;
            var playerPosY = player.Position.Position.GlobalPosition.Y / (planet.Size.Y * Chunk.CHUNKSIZE_Y) *
                             MathHelper.TwoPi;
            ...
            var sunMovement =
                ...
                Matrix.CreateRotationY((float)(MathHelper.TwoPi -
                                               diff.TotalDays * octoDaysPerEarthDay * MathHelper.TwoPi %
                                               MathHelper.TwoPi));

            var sunDirection = Vector3.Transform(sunMovement, new Vector3(0, 0, 1));
            ...
            foreach (var renderer in chunkRenderer)
            {
                ...
                var shift = renderer.GetShift(chunkOffset, planet);
            ...
            entities.Draw(gameTime, camera.View, camera.Projection, chunkOffset,
                new Index2(planet.Size.X, planet.Size.Z));

            if (player.SelectedBox.HasValue)
            {
                // Index3 offset = player.ActorHost.Position.ChunkIndex * Chunk.CHUNKSIZE;
                var offset = camera.CameraChunk * Chunk.CHUNKSIZE;
                var planetSize = planet.Size * Chunk.CHUNKSIZE;
        ...
        private void FillChunkRenderer()
        {
            ...
            if (destinationChunk != currentChunk)
            {
                ...
                for (var y = 0; y < Span; y++)
                {
                    ...
                    for (var z = 0; z < planet.Size.Z; z++)
                        chunkRenderer[rendererIndex, z].SetChunk(localChunkCache, new Index3(local.X, local.Y, z),
                            player.Position.Planet);
                }

                var comparationIndex = player.Position.Position.ChunkIndex;
                orderedChunkRenderer.Sort((x, y) =>
                {
                    if (!x.ChunkPosition.HasValue) return 1;
                    if (!y.ChunkPosition.HasValue) return -1;

                    var distX = comparationIndex.ShortestDistanceXYZ(x.ChunkPosition.Value, planet.Size);
                    var distY = comparationIndex.ShortestDistanceXYZ(y.ChunkPosition.Value, planet.Size);
        ...
        private Index3? CheckPosition(int x, int y, int z)
        {
            ...
            var block = localChunkCache.GetBlock(pos);
