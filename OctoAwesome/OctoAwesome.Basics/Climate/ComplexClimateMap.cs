...
namespace OctoAwesome.Basics.Climate
{
    public class ComplexClimateMap : IClimateMap
    {
        ...
        public float GetTemperature(Index3 blockIndex)
        {
            ...
            var tempFluctuation = tempFluctuationGenerator.GetTileableNoise2D(blockIndex.X, blockIndex.Y,
            ...
            var temperature = tempFluctuation + equatorTemperature +
                              temperatureDifference *
                              (float) Math.Sin((Math.PI / 2) * distance /
                                               equator); //equatorTemperature + distance * temperatureDifference / equator;
            var height = (float) (blockIndex.Z - planet.BiomeGenerator.SeaLevel) /
        ...
        public int GetPrecipitation(Index3 blockIndex)
        {
            ...
            var rawValue = planet.BiomeGenerator.BiomeNoiseGenerator.GetTileableNoise2D(blockIndex.X, blockIndex.Y,
