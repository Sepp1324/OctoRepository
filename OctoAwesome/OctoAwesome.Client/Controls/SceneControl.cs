...
namespace OctoAwesome.Client.Controls
{
    ...
    {
        public static int VIEW_RANGE = 4;
        public const int TEXTURE_SIZE = 64;
        public static int MASK;
        public static int SPAN;
        public static int SPAN_OVER2;
        ...
        protected override void OnPreDraw(GameTime gameTime)
        {
            ...
            var sunMovement = Matrix.CreateRotationX(inclination) * Matrix.CreateRotationY((float)(MathHelper.TwoPi - ((diff.TotalDays * octoDaysPerEarthDay * MathHelper.TwoPi) % MathHelper.TwoPi)));
            ...
            foreach (var renderer in _chunkRenderer)
            {
                ...
                var shift = chunkOffset.ShortestDistanceXY(renderer.ChunkPosition.Value, new Index2(_planet.Size.X, _planet.Size.Y));

                var chunkBox = new BoundingBox(new Vector3(shift.X * Chunk.CHUNKSIZE_X, shift.Y * Chunk.CHUNKSIZE_Y, shift.Z * Chunk.CHUNKSIZE_Z), new Vector3((shift.X + 1) * Chunk.CHUNKSIZE_X, (shift.Y + 1) * Chunk.CHUNKSIZE_Y, (shift.Z + 1) * Chunk.CHUNKSIZE_Z));
        ...
        private void FillChunkRenderer()
        {
            ...
            if (destinationChunk != _currentChunk)
            {
                _localChunkCache.SetCenter(new Index2(_player.Position.Position.ChunkIndex),
                    b =>
                    {
                        if (b)
                        {
                ...
                for (var x = 0; x < SPAN; x++)
                {
                    ...
                    {
                        var local = new Index2(x - SPAN_OVER2, y - SPAN_OVER2) + destinationChunk;
